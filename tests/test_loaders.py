...
def test_elfloader_hexloader_equal():
    ...
    emu1 = rainbow_arm()
    emu1.load("examples/HW_analysis/trezor.elf", verbose=True)
    emu2 = rainbow_arm()
    emu2.load("examples/HW_analysis/trezor.hex", verbose=True)
...
def test_peloader_hacklu2009():
    ...
    emu = rainbow_x86()
    emu.load("examples/hacklu2009/crackme.exe", verbose=True)
